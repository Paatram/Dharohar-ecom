...
export function BagPage() {
  const { cart, giftWrap, giftMessage, setQuantity, removeFromCart, setGiftWrap, setGiftMessage } = useStore();
  ...
  return <div className="bag-page"><section>{lines.map(({ product, quantity }) => <article key={product.slug}><Image src={product.image} alt="" width={130} height={150} /><div><p className="eyebrow">{product.finish}</p><h2><Link href={`/products/${product.slug}`}>{product.name}</Link></h2><strong>{formatInr(product.sellingPricePaise)}</strong><div className="quantity-control"><button type="button" onClick={() => setQuantity(product.slug, quantity - 1)}>−</button><span>{quantity}</span><button type="button" onClick={() => setQuantity(product.slug, quantity + 1)}>+</button><button type="button" onClick={() => removeFromCart(product.slug)}>Remove</button></div></div></article>)}</section><aside><p className="eyebrow">Selection summary</p><div><span>Indicative products</span><strong>{formatInr(subtotal)}</strong></div><label className="gift-toggle"><input type="checkbox" checked={giftWrap} onChange={(event) => setGiftWrap(event.target.checked)} /><span>Add gift presentation <small>Saved privately with this selection.</small></span></label>{giftWrap ? <label className="gift-message">Gift message<textarea value={giftMessage} maxLength={240} rows={4} onChange={(event) => setGiftMessage(event.target.value)} placeholder="Write a message for the recipient" /><small>{giftMessage.length}/240 characters</small></label> : null}<p>GST, shipping and serviceability are excluded until verified. No payment can be accepted from this preview.</p><Link className="button button-wine" href="/checkout-readiness">Review checkout readiness</Link><Link className="text-link" href="/contact">Enquire about the selection →</Link></aside></div>;
}

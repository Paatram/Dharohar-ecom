...
export function AccountDashboard() {
  ...
  const [addressDraft, setAddressDraft] = useState<Record<string, string>>({ label: "Home" });
  ...
  useEffect(() => {
    if (!/^\d{6}$/.test(addressDraft.pincode ?? "")) return;
    const timer = window.setTimeout(() => { setPincodeStatus("Finding city and state…"); void fetch(`/api/commerce/postcode?pincode=${addressDraft.pincode}`).then(async (response) => {
      ...
      setAddressDraft((current) => ({ ...current, city: result.city!, state: result.state! }));
      setPincodeStatus(`${result.city}, ${result.state} selected`);
    }).catch(() => setPincodeStatus("Enter city and state manually.")); }, 250);
  ...
  async function saveAddress(form: HTMLFormElement) {
    ...
    const response = await fetch("/api/commerce/addresses", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({
      ...
      city: values.city, state: values.state, pincode: values.pincode, isDefault: values.isDefault === "on",
    }) });
    const result = await response.json() as { message?: string };
    if (!response.ok) { setMessage(result.message ?? "Address could not be saved."); return; }
    setShowAddress(false); setAddressDraft({ label: "Home" }); setMessage("Address saved securely."); await load();
  }
  ...
  return <div className="account-dashboard">
    ...
    <section><div className="account-section-head"><div><p className="eyebrow">Addresses</p><h2>Saved delivery details</h2></div><button className="button button-outline" type="button" onClick={() => setShowAddress(!showAddress)}>Add address</button></div>
      {showAddress ? <form className="brief-form compact-form" onSubmit={(event) => { event.preventDefault(); void saveAddress(event.currentTarget); }}><div className="form-grid"><label>Label<input name="label" value={addressDraft.label ?? ""} onChange={(event) => setAddressDraft({ ...addressDraft, label: event.target.value })} required /></label><label>Recipient<input name="recipientName" autoComplete="name" required /></label><label>Phone<input name="phone" autoComplete="tel" required /></label><label>Address line 1<input name="line1" autoComplete="address-line1" required /></label><label>Apartment, floor or landmark (optional)<input name="line2" autoComplete="address-line2" /></label><label>Pincode<input name="pincode" value={addressDraft.pincode ?? ""} onChange={(event) => setAddressDraft({ ...addressDraft, pincode: event.target.value.replace(/\D/g, "").slice(0, 6) })} pattern="[0-9]{6}" inputMode="numeric" required />{pincodeStatus ? <small className="field-status">{pincodeStatus}</small> : null}</label><label>City<input name="city" value={addressDraft.city ?? ""} onChange={(event) => setAddressDraft({ ...addressDraft, city: event.target.value })} required /></label><label>State<input name="state" value={addressDraft.state ?? ""} onChange={(event) => setAddressDraft({ ...addressDraft, state: event.target.value })} required /></label></div><label className="consent-field"><input name="isDefault" type="checkbox" /><span>Use as my default address</span></label><button className="button button-wine" type="submit">Save address</button></form> : null}

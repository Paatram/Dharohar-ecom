...
export function HeaderCommerceActions() {
  ...
  return <div className="commerce-actions">
    <button type="button" onClick={() => setSearchOpen(true)} aria-label="Search Dharohar"><span aria-hidden="true">⌕</span><small>Search</small></button>
    <Link href="/wishlist" aria-label={`Wishlist with ${wishlist.length} items`}><span aria-hidden="true">♡</span><small>Saved {wishlist.length || ""}</small></Link>
    <Link href="/compare" aria-label={`Compare ${compare.length} items`}><span aria-hidden="true">⇄</span><small>Compare {compare.length || ""}</small></Link>
    <button type="button" onClick={() => setCartOpen(true)} aria-label={`Open selection bag with ${cartCount} items`}><span aria-hidden="true">▢</span><small>Bag {cartCount || ""}</small></button>
  </div>;
}
...
function SearchDialog() {
  ...
  return <div className="overlay-shell search-overlay" role="dialog" aria-modal="true" aria-label="Search products">
    ...
    <section className="search-panel">
      <header><p className="eyebrow">Find an object</p><button type="button" onClick={() => setSearchOpen(false)} aria-label="Close search">×</button></header>
...
function CartDrawer() {
  ...
  return <div className={`drawer-layer ${cartOpen ? "drawer-layer-open" : ""}`} aria-hidden={!cartOpen}>
    ...
    <aside className="cart-drawer" aria-label="Selection bag">
      <header><div><p className="eyebrow">Your selection</p><h2>Bag</h2></div><button type="button" onClick={() => setCartOpen(false)} aria-label="Close bag">×</button></header>
      ...
      {lines.length ? <footer>
        <label className="gift-toggle"><input type="checkbox" checked={gift} onChange={(event) => setGift(event.target.checked)} /><span>Add gift presentation and a message <small>Availability and price confirmed before launch.</small></span></label>
